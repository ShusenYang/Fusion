/**
 * \file
 *         This implementation of bcp_queue (see \ref bcp_queue.h) uses FIFO scheduling. 
 */
#include "bcp_queue.h"
#include "bcp.h"
#include <stdbool.h>
...
static void bcp_queue_print(struct bcp_queue *s){
        ...
        int j = 0;
        for(i =  bcp_queue_top(s); i != NULL; i= list_item_next(i)){
            PRINTF("DEBUG: Queue item#%d=%p node[%d].[%d]\n", j++, i, i->hdr.origin.u8[0], i->hdr.origin.u8[1]);
        }
    #endif
}