 ...
 */
static bool isFusionPacket(struct bcp_queue *s, struct bcp_queue_item* itm){

    //If it is a fusion packet 
    return (itm->hdr.origin.u8[0] == 250 
            && itm->hdr.origin.u8[1] == 250);
}
 ...
 * 
 * @param s the queue
 * @param itm the new item 
 * @return the queue item which where the given itm should be inserted after
 */
static struct bcp_queue_item* findPacketLocation(struct bcp_queue *s, struct bcp_queue_item* itm){
    
    struct bcp_queue_item* result = NULL;
    struct bcp_queue_item* preItem = NULL;
    ...
    struct bcp_queue_item * i;
        for(i =  bcp_queue_top(s); i != NULL; i= list_item_next(i)){
            ...
            //Finds the fusion packet which has the least higher fusion count.
            if(isFusionPacket(s,i)){
               memcpy(&r, &i->data, 2);
               //If only if the given itm is a fusion packet
               if(f > r && isFusionPacket(s,itm)){
                   result = preItem;
                   break;
               }
            }else{
                //Otherwise, return the last fusion packet i.e. before normal packets
                 result = preItem;
                break;
            }
            preItem = i;
...
struct bcp_queue_item * bcp_queue_push(struct bcp_queue *s, struct bcp_queue_item *i){
    ...
    newRow->hdr.packet_length = i->hdr.packet_length;   
     ...
     struct bcp_queue_item * insertAfter = findPacketLocation(s,newRow );
    ...
    if(insertAfter == NULL){
        ...
        list_insert(*s->list, insertAfter, newRow);
    }
...
static void bcp_queue_print(struct bcp_queue *s){
        ...
        for(i =  bcp_queue_top(s); i != NULL; i= list_item_next(i)){
            memcpy(&r, &i->data, 2);
            printf("DEBUG: Queue item#%d=%p node[%d].[%d] data=%x\n", j++, i, i->hdr.origin.u8[0], i->hdr.origin.u8[1], r);
        }
    #endif
}